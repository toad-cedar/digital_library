...
class OfflineFolder(Base):
  ...
  parent_folder_id: Mapped[int | None] = mapped_column(ForeignKey('offline_folders.id')) # Рекурсивная связь
  created_at:       Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
  
  user          = relationship("User", back_populates="offline_folders")
  parent_folder = relationship("OfflineFolder", remote_side=[id], back_populates="child_folders")
